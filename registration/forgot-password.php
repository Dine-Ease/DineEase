...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users1 WHERE email='{$email}'")) > 0) {
        $query = mysqli_query($conn, "UPDATE users1 SET code='{$code}' WHERE email='{$email}'");
        ...
        if ($query) {
            ...
            try {
                ...
                $mail->Body = 'Here is the verification link <b><a href="dineease.000webhostapp.com/registration/change-password.php?reset=' . $code . '">dineease.000webhostapp.com/registration/change-password.php?reset=' . $code . '</a></b>';
